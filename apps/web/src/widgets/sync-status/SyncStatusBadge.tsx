...
export const SyncStatusBadge = () => {
  ...
  if (isSyncing) {
    return (
      <span className="rounded bg-blue-100 px-2 py-0.5 text-xs font-medium text-blue-900">Syncing…</span>
    );
  }
  if (lastError) {
    return (
      <span
        className="rounded bg-red-100 px-2 py-0.5 text-xs font-medium text-red-900"
        title={lastError}
      >
        Sync issue
      </span>
