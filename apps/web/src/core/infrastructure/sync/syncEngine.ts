...
import { checkpointStore } from "@/core/infrastructure/sync/checkpoints";
import { remapLocalUserIdToServer } from "@/core/infrastructure/sync/userIdRemap";
...
const collectPushPayload = async (
  userId: string,
  lastPushAt: number,
): Promise<SyncPushBody> => {
  ...
  const [users, projects, tasks, checklistItems] = await Promise.all([
    ...
    db.projects
      .find({ selector: { userId, updatedAt: { $gt: lastPushAt } } })
      .exec(),
...
const mergePulledData = async (payload: SyncPullResponse) => {
  ...
  await lwwMerge<User>(
    payload.users,
    (doc) => db.users.upsert(doc),
    (id) => db.users.findOne(id).exec(),
  );
  await lwwMerge<Project>(
    payload.projects,
    (doc) => db.projects.upsert(doc),
    (id) => db.projects.findOne(id).exec(),
  );
  await lwwMerge<Task>(
