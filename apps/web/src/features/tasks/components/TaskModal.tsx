...
import { useChecklistItemsForTask } from "@/features/tasks/hooks/useChecklistItemsForTask";
import {
  Accordion,
  AccordionContent,
  AccordionItem,
...
export const TaskModal = ({ userId, projectId, tasks }: Props) => {
  ...
  const checklistItemsEnabled = taskModalOpen && taskModalMode === "edit" ? true : false;
  ...
  const derivedStatus = useMemo(() => deriveTaskStatus(checklistItems), [checklistItems]);

  const stepsText = useMemo(() => stepsCountText(checklistItems.length), [checklistItems.length]);
  ...
  useEffect(() => {
    ...
    if (taskModalMode !== "edit") return;
    if (!selectedTask) return;
    schedule("task:description", () => void saveTaskDescription(editDescription), 250);
  }, [
    editDescription,
    saveTaskDescription,
    schedule,
  ...
  const addNewItem = useCallback(async () => {
    ...
    setNewDraft({ title: "", description: "", status: "NO_STARTED" });
  }, [newDraft.description, newDraft.status, newDraft.title, selectedTaskId, userId]);
          ...
          <form onSubmit={save}>
            ...
            <div className="px-5 pb-4 pt-4">
              <FieldGroup>
                ...
                <Field>
                  <FieldLabel htmlFor="task-create-description">Description</FieldLabel>
                  <Input
                ...
                <div className="min-w-0">
                  <div className="flex items-start gap-2">
                    ...
                    <div className="flex flex-col">
                      <h1 className="truncate">{selectedTask?.title ?? "Task"}</h1>
                      <div
                        className={cn(
                  ...
                  <AccordionContent className="pb-0">
                          ...
                          return (
                            <li key={item.id} className="py-2">
                              <button
                                ...
                                onClick={() =>
                                  setActiveChecklistId((cur) => (cur === item.id ? null : item.id))
                                }
                              >
                                ...
                                <div className="min-w-0 flex-1">
                                  ...
                                  <div className="mt-1 flex min-w-0 items-center gap-2 text-xs text-muted-foreground">
                                    <span className="inline-flex items-center gap-1.5">
                                      ...
                                      />
                                      <span className={STATUS_UI[item.status].textClass}>
                              ...
                              {isActive && (
                                <div className="mt-2 rounded-md border bg-background p-3">
                                  <ChecklistItemFormFields
                                    draft={draft}
                                    onDraftChange={(patch) => applyChecklistPatch(item, patch)}
                                  />
                                </div>
                              )}
                            </li>
                    ...
                    <div className="mt-2">
                      ...
                      >
                        ...
                        <div className="text-sm font-semibold text-blue-600">ADD NEW ITEM</div>
                      </button>
                      {addingOpen && (
                        <div className="mt-2 rounded-md border p-3">
                          <ChecklistItemFormFields
                            draft={newDraft}
                            onDraftChange={(patch) => setNewDraft((d) => ({ ...d, ...patch }))}
                            footer={
