...
export class ChecklistRepository {
  ...
  async softDeleteChecklistItemsForTask(
    userId: string,
    taskId: string,
  ): Promise<void> {
    const db = await getDatabase();
  ...
  getChecklistItemsByTaskSnapshot(
    userId: string,
    taskId: string,
  ): ChecklistItem[] {
    return (
      this.checklistByTaskCache.get(byTaskKey(userId, taskId)) ??
      ChecklistRepository.EMPTY
    );
  }

  getChecklistItemsByTasksSnapshot(
    userId: string,
    taskIds: string[],
  ): ChecklistItem[] {
    if (taskIds.length === 0) return ChecklistRepository.EMPTY;
    return (
      this.checklistByTasksCache.get(byTasksKey(userId, taskIds)) ??
      ChecklistRepository.EMPTY
    );
  }
